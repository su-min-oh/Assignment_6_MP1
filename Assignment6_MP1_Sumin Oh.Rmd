...
```{r}
#1.Naive : This model forecasts the futre values based on the last value of the data. So the forecast values for 12 months are 4.87
FV_naive<-naive(FV_ts,h=12)
FV_naive

#2.Average : This model predicts the future value based on the average of given dataset. So the forecast for the next 12 months are the same.
FV_avg<-meanf(FV_ts,h=12)
FV_avg

#3.Exponential Smoothing : This is done with HW model without beta(trend) and gamma(seasonality).As you can see from the plot below, this makes the given time series graph smoother.
FV_SES<- HoltWinters(FV_ts,beta=FALSE,gamma=FALSE)
FV_SES_FC <- forecast(FV_SES,h=12)
plot(FV_SES)

#4.HoltWinters : HoltWinters model uses level, trend, and seasonality for forecasting.  
FV_HW<-hw(FV_ts,h=12)
FV_HW

#5.Decomposition : With stl, we can decompose the data into factors such as seasonal, trend, and remainder.(plot)  
